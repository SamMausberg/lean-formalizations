...
This is the top-level problem statement file for the sunflower conjecture.  The rest of the
`E20/` directory contains proved components intended to be tied into this statement.
...
/--
A sunflower `F` with kernel `S` is a collection of sets in which all possible distinct pairs of
sets share the same intersection `S`.
-/
...
noncomputable def f (n k : ℕ) : ℕ :=
  sInf {m : ℕ | ∀ {α : Type}, ∀ F : Set (Set α),
    ((∀ f ∈ F, f.ncard = n) ∧ m ≤ F.ncard) →
...
/--
The top-level exponential sunflower conjecture for Erdős problem 20.

This is separated as a named proposition so future work can replace the current `Iff.rfl`
problem-statement wrapper by a genuine proof of the right-hand side.
-/
def erdos20Conjecture : Prop :=
  ∃ c : ℕ → ℕ, ∀ n k, n > 0 → f n k < (c k) ^ n

/--
Is it true that `f(n,k) < c_k^n` for some constant `c_k > 0` and for all `n > 0`?

The theorem is intentionally only the checked problem-statement wrapper.  It contains no proof
placeholder and does not pretend that the conjecture has already been proved.
-/
@[category research open_problem, AMS 5]
theorem erdos_20 : answer(erdos20Conjecture) ↔
